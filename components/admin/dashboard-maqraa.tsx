  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0 font-sans" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          { label: dm?.totalRecitations ?? (isAr ? 'إجمالي التلاوات' : 'Total recitations'), value: totalRecitations, icon: ClipboardList, circle: "bg-primary/10 border-primary/20", iconColor: "text-primary" },
          { label: dm?.masteredRecitations ?? (isAr ? 'تلاوات متقنة' : 'Mastered recitations'), value: masteredCount, icon: CheckCircle2, circle: "bg-emerald-500/10 border-emerald-500/20", iconColor: "text-emerald-600 dark:text-emerald-400" },
          { label: dm?.inReview ?? (isAr ? 'قيد المراجعة' : 'In review'), value: inReviewCount, icon: Clock, circle: "bg-amber-100/50 dark:bg-amber-900/30 border-amber-200 dark:border-amber-900/50", iconColor: "text-amber-700 dark:text-amber-400" },
