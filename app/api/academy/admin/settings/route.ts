...
const ACADEMY_SETTING_PREFIXES = [
...
function resolveSettingType(key: string): string | null {
  const prefix = ACADEMY_SETTING_PREFIXES.find((candidate) => key.startsWith(candidate))
  if (!prefix) return null
  return prefix.startsWith("academy_fiqh_") ? "academy_forum" : prefix.slice(0, -1)
}
  ...
  try {
    const settings = await query(
      `SELECT s.setting_key,
        ...
        WHERE ${ACADEMY_SETTING_PREFIXES.map((_, index) => `s.setting_key LIKE $${index + 1}`).join(" OR ")}
        ORDER BY s.setting_key`,
      ACADEMY_SETTING_PREFIXES.map((prefix) => `${prefix}%`)
    )
