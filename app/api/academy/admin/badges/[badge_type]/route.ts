...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ badge_type: string }> }) {
  const session = await getSession()
  if (!session || !['academy_admin', 'admin'].includes(session.role)) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  }
  const { badge_type } = await params
  try {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ badge_type: string }> }) {
  const session = await getSession()
  if (!session || !['academy_admin', 'admin'].includes(session.role)) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  }
  const { badge_type } = await params
  try {
    await query(`DELETE FROM badge_definitions WHERE badge_type = $1`, [badge_type])
