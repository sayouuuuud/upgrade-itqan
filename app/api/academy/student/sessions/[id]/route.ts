...
export async function GET(
  ...
  try {
    ...
    const userAttendance = await queryOne<{ id: string; joined_at: string; left_at: string | null }>(
      `SELECT id, joined_at, left_at FROM session_attendance WHERE session_id = $1 AND user_id = $2`,
      [id, session.sub]
    )
  ...
  try {
    ...
    if (action === 'join') {
      ...
      const existing = await queryOne<{ id: string }>(
        `SELECT id FROM session_attendance WHERE session_id = $1 AND user_id = $2`,
        [id, session.sub]
      )
      ...
      await query(
        `INSERT INTO session_attendance (session_id, user_id, joined_at) VALUES ($1, $2, NOW())`,
        [id, session.sub]
      )
    ...
    if (action === 'leave') {
      await query(
        `UPDATE session_attendance SET left_at = NOW() WHERE session_id = $1 AND user_id = $2 AND left_at IS NULL`,
        [id, session.sub]
      )
