...
export async function GET(req: NextRequest) {
  try {
    ...
    // Reader sees paths they created OR are assigned to manage
    let paths: any[] = []
    try {
      paths = (await query(
        `SELECT p.*, u.name AS created_by_name, m.name AS manager_name, m.email AS manager_email
          ...
          WHERE (p.created_by = $1 OR p.manager_id = $1)
    ...
    console.error("[reader tajweed paths GET]", err)
    return NextResponse.json({ error: "حدث خطأ" }, { status: 500 })
  }
}

// POST /api/reader/tajweed-paths
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[reader tajweed paths POST]", err)
