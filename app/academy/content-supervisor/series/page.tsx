    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-3">
            ...
            {isAr ? 'مراجعة السلاسل' : 'Review Series'}
          </h1>
          <p className="text-muted-foreground text-sm flex items-center gap-2">
            <Sparkles className="w-4 h-4" />
            {isAr ? 'مراجعة سلاسل الأكاديمية قبل نشرها للطلاب' : 'Review academy series before publishing to students'}
          </p>
        </div>
      </div>
          ...
          <div>
            ...
            <p className="text-sm text-amber-800/80 dark:text-amber-500/80">
              {isAr 
                ...
                : <>Please run the database script <code className="font-mono bg-amber-500/20 px-1.5 py-0.5 rounded text-amber-900 dark:text-amber-300">scripts/030-content-review-workflow.sql</code> to enable the series review workflow.</>}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-card border border-border/50 rounded-2xl shadow-sm p-4 space-y-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={isAr ? 'ابحث بعنوان السلسلة أو اسم المعلم...' : 'Search by series title or teacher name...'}
            value={search}
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-16 text-center flex flex-col items-center justify-center shadow-sm">
          ...
          <h3 className="text-lg font-bold text-foreground mb-2">{isAr ? 'لا توجد سلاسل حالياً' : 'No series found'}</h3>
          <p className="text-sm text-muted-foreground max-w-sm">
            {searchDebounced 
              ? (isAr ? 'لم نعثر على أي نتائج مطابقة لبحثك، جرب كلمات مفتاحية أخرى.' : 'No matching results found. Try other keywords.')
              : (isAr ? 'لا توجد سلاسل في هذا القسم حالياً، ستظهر السلاسل هنا عند إرسالها للمراجعة.' : 'No series in this section currently. Series will appear here when submitted for review.')}
          </p>
        </div>
      ) : (
              ...
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    {s.total_items} {isAr ? 'عنصر' : 'items'}
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5" />
                    {new Date(s.submitted_for_review_at || s.created_at).toLocaleDateString(isAr ? 'ar-EG' : 'en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                  </span>
                </div>
              </div>
...
function StatusBadge({ status, isAr }: { status: string; isAr: boolean }) {
  const config: Record<string, { labelAr: string; labelEn: string; cls: string; Icon: any }> = {
    ...
    draft: { labelAr: 'مسودة', labelEn: 'Draft', cls: 'bg-muted text-muted-foreground border border-border/50', Icon: Library },
  }
