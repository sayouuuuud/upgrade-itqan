...
import {
  ...
  User, ChevronLeft, Loader2, AlertCircle,
} from 'lucide-react'
...
export default function ContentPathsListPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-foreground">مسارات المقرئ</h1>
        ...
        <div className="flex items-start gap-3 bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              شغّل سكربت قاعدة البيانات <code className="font-mono">scripts/030-content-review-workflow.sql</code> لتفعيل مراجعة المسارات.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="بحث بعنوان المسار أو اسم المقرئ..."
          value={search}
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border rounded-2xl p-16 text-center">
          <RouteIcon className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; cls: string; Icon: any }> = {
    ...
    draft: { label: 'مسودة', cls: 'bg-muted text-muted-foreground', Icon: RouteIcon },
  }
