    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-3">
            ...
            {isAr ? 'مراجعة المسارات' : 'Review Paths'}
          </h1>
          <p className="text-muted-foreground text-sm flex items-center gap-2">
            <Sparkles className="w-4 h-4" />
            {isAr ? 'مراجعة مسارات التجويد والحفظ المرسلة من المقرئين قبل نشرها' : 'Review Tajweed and Memorization paths submitted by reciters before publishing'}
          </p>
        </div>
      </div>
          ...
          <div>
            ...
            <p className="text-sm text-amber-800/80 dark:text-amber-500/80">
              {isAr 
                ...
                : <>Please run the database script <code className="font-mono bg-amber-500/20 px-1.5 py-0.5 rounded text-amber-900 dark:text-amber-300">scripts/030-content-review-workflow.sql</code> to enable the paths review workflow.</>}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-card border border-border/50 rounded-2xl shadow-sm p-4 space-y-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={isAr ? 'ابحث بعنوان المسار أو اسم المقرئ...' : 'Search by path title or reciter name...'}
            value={search}
...
function StatusBadge({ status, isAr }: { status: string; isAr: boolean }) {
  const config: Record<string, { labelAr: string; labelEn: string; cls: string; Icon: any }> = {
    ...
    draft: { labelAr: 'مسودة', labelEn: 'Draft', cls: 'bg-muted text-muted-foreground border border-border/50', Icon: RouteIcon },
  }
