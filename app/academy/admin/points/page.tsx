...
export default function AdminPointsPage() {
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      const res = await fetch('/api/academy/admin/points')
      if (res.ok) {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true)
    try {
      const res = await fetch(`/api/academy/admin/points?search=${encodeURIComponent(searchQuery)}`)
      if (res.ok) {
        const json = await res.json()
  ...
  const selectStudent = async (userId: string) => {
    ...
    try {
      const res = await fetch(`/api/academy/admin/points?user_id=${userId}`)
      if (res.ok) {
        const json = await res.json()
