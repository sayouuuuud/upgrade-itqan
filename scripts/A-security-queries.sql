...
-- ////////////////////////////////////////////////////////////////////////////
-- SECTION 2 — REFERENCE TEMPLATES (do NOT execute as-is)
-- ////////////////////////////////////////////////////////////////////////////
--
-- The queries below are executed from application code with parameter
-- bindings ($1, $2, ...). They are kept here as documentation. Do not paste
-- them into the SQL editor — they will fail with "there is no parameter $1".
-- To run one manually, replace `$1` with a literal value (e.g. a quoted UUID).


-- NOTE: We use line comments (`--`) instead of block comments (`/* */`)
-- because PostgreSQL block comments are nested-aware, and any literal
-- `/*` (e.g. inside the strings 'DENY - /academy/student/*') would open
-- an inner comment, leaving the outer one unterminated.

-- ----------------------------------------------------------------------------
-- A-4: Fetch fresh permission flags from DB (used by middleware/route handlers)
-- ----------------------------------------------------------------------------
-- SELECT
--   u.id,
--   u.role,
